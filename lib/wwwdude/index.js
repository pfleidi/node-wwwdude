...
var createClient = exports.createClient = function createClient(clientOptions) {
  ...
  var clientHeaders = Util.mergeAttributes(defaultHeaders, clientOptions.headers);
  ...
  function makeRequest(method, url, options) {
    ...
    if (options.payload) {
      ...
      request = client.request(method, parsedUrl.path, options.headers);
      request.write(options.payload, encoding);
    } else {
      request = client.request(method, parsedUrl.path, options.headers);
    }
     ...
     * @api private
     */
    _decodeGzip = function _decodeGzip(response) {
      var body = '';
     ...
     * @api private
     */
    _redirect = function _redirect(response) {
  ...
  return {
    ...
    get: function (url, requestHeaders) {
      var headers = Util.mergeAttributes(clientHeaders, requestHeaders);
